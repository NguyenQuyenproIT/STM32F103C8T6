...
// Hàm này dùng de doc 2 byte du lieu ánh sáng tu cam bien BH1750 qua I2C.
// giao tiep lay data tu sensor và chuyen data sang LUX
uint16_t BH1750_ReadLight(){
  uint16_t value = 0;
    ...
    I2C_GenerateSTART(I2C1, ENABLE);// tao tin hieu start
    while (!I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_MODE_SELECT)); // xac nhan start thanh cong (mode MASTER)
		// SDA 1 -> 0 truoc khi CLK 1 -> 0) [clk = 1]
	
    I2C_Send7bitAddress(I2C1, BH1750_ADDR, I2C_Direction_Receiver); // gui 7 bit dia chi BH1750 va 1 bit 0x01 la Read
    while (!I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED)); // Cho gui xong, xac nhan vao che do Read (slave ack, stm32 chuyen sang che do nhan du lieu)

		//Doc du lieu 2 byte
    while (!I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_BYTE_RECEIVED)); // cho rxne = 1, doc du lieu tu reg DR, sau khi doc xong, do ACK dang enable nên stm32 tu gui ACK
    msb = I2C_ReceiveData(I2C1); // bit cao

    while (!I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_BYTE_RECEIVED)); // similar
    lsb = I2C_ReceiveData(I2C1); // bit thap

    I2C_AcknowledgeConfig(I2C1, DISABLE); // Tat ACK de bao rang stm ko nhan them du lieu (noti cho slave byte tiep theo la byte cuoi cung master rv)
    I2C_GenerateSTOP(I2C1, ENABLE); // ket thuc giao tiep
    I2C_AcknowledgeConfig(I2C1, ENABLE); // Bat ACK lai de chuan bi cho lan do sau

    value = ((msb << 8) | lsb) / 1.2; // chuyen ve Lux (bh1750 tra ve gia tri 16 bit)
    return value;
}